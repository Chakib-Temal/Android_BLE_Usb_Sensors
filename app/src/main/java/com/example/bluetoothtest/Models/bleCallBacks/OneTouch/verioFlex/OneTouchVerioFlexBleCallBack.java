package com.example.bluetoothtest.Models.bleCallBacks.OneTouch.verioFlex;
...
public class OneTouchVerioFlexBleCallBack extends BleModelCallBack {

    public static final String TAG  = OneTouchVerioFlexBleCallBack.class.getSimpleName();
    ...
    public void onConnectSuccess(final BleDevice bleDevice, BluetoothGatt gatt, int status) {
        ...
        MainActivity.writeTextSucces("Connexion avec succès avec OneTouch Verio Flex");
        mDevice = bleDevice;
    ...
    public static BleNotifyCallback oneTouchVersioFlexCallBack = new BleNotifyCallback() {
        ...
        public void onCharacteristicChanged(byte[] data) {
            ...
            try {
                int res = VerioFlexHelper.parseMessage(data);

                if (res == 1){
                    writeRXCharacteristic(VerioFlexHelper.getTcounterCMD());
                }

                else if (res == 2){
                    writeRXCharacteristic(VerioFlexHelper.getRcounterCMD());
                }

                else if (res == 3){

                    OneTouchVerioFlexBleCallBack.writeRXCharacteristic(VerioFlexHelper.getRecordCMD(VerioFlexHelper.highest_record_number) );
                }

                else if (res == 4){
                    Log.i(VerioFlexHelper.TAG, "finish");
